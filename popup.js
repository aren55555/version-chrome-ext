document.addEventListener('DOMContentLoaded', function() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    chrome.tabs.sendMessage(tabs[0].id, {action: 'getVersionInfo'}, function(response) {
      ...
      if (response && response.version) {
        displayVersionInfo(response.version);
      } else if (response && response.isJson) {
        statusText.textContent = 'JSON page found, but no "version" field detected';
        statusText.className = 'no-version';
      } else {
        statusText.textContent = 'No JSON version info found on this page';
        statusText.className = 'no-version';
      }
    });
  });
  ...
  function displayVersionInfo(version) {
    statusText.innerHTML = `<span class="version-detected">Version detected:</span><br><code>${version}</code>`;
